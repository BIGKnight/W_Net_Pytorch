...
def eval_model(config, eval_loader, modules):
    net = modules['model'].eval()
    
    net_encoder = list(net.children())[0]
    net_attention = list(net.children())[-2]
    ...
    for eval_img_index, eval_img, eval_gt in eval_loader:
            ...
            with torch.no_grad():
#                 B5_C3, B4_C3, B3_C3, B2_C2 = net_encoder(eval_patchs)
#                 eval_prediction = net_attention(B5_C3, B4_C3, B3_C3, B2_C2)
